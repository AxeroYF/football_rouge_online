...
test("联赛旧数据冲突时后台提供开启全新赛季的恢复入口", () => {
  ...
  assert.match(source, /error\.status === 409 && error\.code === "YDL_SEASON_DATA_CONFLICT"/);
  assert.match(source, /function renderLeagueUnavailable\(error\)/);
  assert.match(source, /SERVICE UNAVAILABLE/);
  ...
  assert.match(source, /league-predictions-today/);
  assert.match(source, /league-prediction-monitor/);
  assert.match(source, /预测系统监测/);
  assert.match(source, /\/api\/admin\/league\/predictions\/today/);
  assert.match(source, /今日玩家投注明细/);
  assert.match(source, /未结算本金/);
});
...
test("管理员登录后可以读取去敏玩家列表和竞技统计", async () => {
  ...
  const predictions = await request("/api/admin/league/predictions/today", { token:login.value.token });
  assert.equal(predictions.statusCode, 200);
  assert.match(predictions.value.predictions.dateKey, /^\d{4}-\d{2}-\d{2}$/);
  assert.equal(predictions.value.predictions.timezone, "Asia/Shanghai");
  assert.ok(Array.isArray(predictions.value.predictions.players));
  assert.equal(typeof predictions.value.predictions.totals.realizedNet, "number");
