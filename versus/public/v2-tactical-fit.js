...
function inPossessionFit(plan, groups, wide) {
  ...
  const profiles = {
    ...
    wideOverload:() => squadMetric(wide, { progression:.28, chanceCreation:.34, movement:.24, pressResistance:.14 }) * .72 + squadMetric(groups.ATT, { aerialFinishing:.55, movement:.25, finishing:.2 }) * .28,
    ...
    longBall:() => squadMetric([...groups.DEF, ...groups.MID], { buildUp:.38, progression:.38, pressResistance:.24 }) * .6 + squadMetric(groups.ATT, { aerialFinishing:.46, movement:.32, finishing:.22 }) * .4,
  };
