...
import { moveRealPlayerToPool, REAL_PLAYER_BY_ID, REAL_PLAYERS } from "./player-pool.js";
...
export function ydlContentView() {
  return {
    ...
    players:REAL_PLAYERS.map((player) => ({
      id:player.id,
      name:player.name,
      ...
      isLegend:Boolean(player.legendAbility),
    })),
    traits:YDL_TRAIT_CARDS.map((trait) => ({
      id:trait.id,
      name:trait.name,
...
export async function updateYdlTrait(id, patch) {
  const trait = YDL_TRAIT_BY_ID[id];
  if (!trait) throw Object.assign(new Error("特性卡不存在"), { statusCode:404 });
  const allowed = {
    name:patch.name,
