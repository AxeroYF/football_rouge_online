...
export async function handleVersusApi(request, response, pathname, readJson, sendJson) {
  ...
  if (offlineYdl) {
    const disabledLeagueFeature = [
      "/api/versus/league/friendlies/",
      "/api/versus/league/ai-training/",
      "/api/versus/league/mirror-marketplace/",
    ].some((prefix) => pathname.startsWith(prefix));
    const allowed = pathname.startsWith("/api/versus/league")
      || ["/api/versus/live", "/api/versus/activity", "/api/versus/broadcasts", "/api/versus/profile"].includes(pathname)
      || pathname === "/api/versus/watch" && isLeagueBroadcast(body.code)
      || /^\/api\/versus\/watch\/YDL-/i.test(pathname)
      || pathname.endsWith("/leave-watch") && isLeagueBroadcast(code);
    if (!allowed || disabledLeagueFeature) return sendJson(response, 404, { ok:false, error:"该功能未包含在YDL单机版中" });
  }
  let result;
  if (pathname.startsWith("/api/versus/league")) {
    ...
    else if (pathname === "/api/versus/league/season-final") result = { seasonFinalTournament:yellowDogsLeague.seasonFinalTournamentView(account) };
    else if (pathname === "/api/versus/league/season-final/ban") { yellowDogsLeague.submitSeasonFinalBan(account, body.selection); result = { league:yellowDogsLeague.view(account, { developer, includePlayerDirectory:false }) }; }
    else if (pathname === "/api/versus/league/season-final/simulate" && developer) result = { seasonFinalTournament:yellowDogsLeague.simulateNextSeasonFinalMatch() };
    ...
    else if (pathname === "/api/versus/league/team/tactical-shape-preview") result = { tacticalShapePreview:yellowDogsLeague.tacticalShapePreview(account, body) };
    ...
    else if (pathname === "/api/versus/league/club/stadium") result = { league:yellowDogsLeague.updateClubStadium(account, body) };
    else if (pathname === "/api/versus/league/club/stadium/expand") result = { league:yellowDogsLeague.expandClubStadium(account) };
    else if (pathname === "/api/versus/league/club/sponsor/respond") result = { league:yellowDogsLeague.respondClubSponsorOffer(account, body) };
    ...
    else if (pathname === "/api/versus/league/mirror-marketplace/batch-simulate") result = yellowDogsLeague.createMirrorBatchSimulation(account, body);
    else if (pathname === "/api/versus/league/mirror-marketplace/compute-node/save") result = yellowDogsLeague.saveMirrorComputeNode(account, body);
    else if (pathname === "/api/versus/league/mirror-marketplace/compute-node/remove") result = yellowDogsLeague.removeMirrorComputeNode(account, body.nodeId);
    ...
    else if (pathname === "/api/versus/league/shop/buy-meteor-stand") result = { league:yellowDogsLeague.buyMeteorStand(account, { compact:true }) };
    ...
    else if (pathname === "/api/versus/league/packs/choose") result = { league:yellowDogsLeague.chooseS4Pack(account, body.offerId, body.choiceId ?? body.leaguePlayerId, { compact:true }) };
    else if (pathname === "/api/versus/league/cosmetics/equip") result = { league:yellowDogsLeague.equipCosmetic(account, body.itemId, body.slot, { compact:true }) };
    ...
    else if (pathname === "/api/versus/league/market/list-cosmetic") result = { league:yellowDogsLeague.listCosmetic(account, body.cosmeticItemId, body.price, { compact:true }) };
    ...
    else if (pathname === "/api/versus/league/player/release") result = { league:yellowDogsLeague.releasePlayer(account, body.leaguePlayerId, { compact:true }) };
    else if (pathname === "/api/versus/league/card/release") result = { league:yellowDogsLeague.releaseCard(account, body.cardId, body.confirmOwnershipReturn === true, { compact:true }) };
    else if (pathname === "/api/versus/league/cards/release") result = { league:yellowDogsLeague.releaseCards(account, body.cardIds, { compact:true }) };
    ...
    else if (pathname === "/api/versus/league/card/enhance") result = { enhancement:yellowDogsLeague.enhanceS4Card(account, body.mainCardId, body.materialCardId, body.useProtection === true, { compact:true, requestId:body.requestId }) };
    else if (pathname === "/api/versus/league/card/enhance-batch") result = { batchEnhancement:yellowDogsLeague.batchEnhanceS4Cards(account, body.batchPlayerId, body.mainLevel, body.materialLevel, body.quantity, { compact:true }) };
    ...
    else if (pathname === "/api/versus/league/ownership/return") result = { league:yellowDogsLeague.returnOwnership(account, body.leaguePlayerId, { compact:true }) };
    else if (pathname === "/api/versus/league/ownership/return-batch") result = { league:yellowDogsLeague.returnOwnerships(account, body.leaguePlayerIds, { compact:true }) };
  ...
  else if (request.method === "POST" && pathname === "/api/versus/activity") {
    versusRooms.account(body.playerId, body.accountToken);
    result = { active:true };
  }
  ...
  else if (request.method === "POST" && pathname === "/api/versus/profile/password") result = versusRooms.changePassword(body.playerId, body.accountToken, body.currentPassword, body.nextPassword);
