...
test("分片每日重启归档后可以重载且既有归档不会重复写入", () => {
  const directory = mkdtempSync(path.join(process.cwd(), ".tmp-ydl-sharded-daily-reset-"));
  const statePath = path.join(directory, "league-state");
  let currentTime = Date.parse("2026-08-12T23:59:00+08:00");
  try {
    const service = new YellowDogsLeagueService({ statePath, backupDir:null, now:() => currentTime, rng:() => .37 });
    service.state.season.status = "completed";
    service.state.season.completedAt = currentTime - 60_000;
    ...
    currentTime = Date.parse("2026-08-13T00:01:00+08:00");
    service.resetDailyCompetitions({ skipBackup:true, skipHonorRoomUpdate:true, skipView:true });
    const archiveReference = service.shardStore.manifest.shards.archives.entries[0].path;
    const archiveContents = readFileSync(path.join(statePath, archiveReference), "utf8");
    service.save({ scopes:["core"], skipDailyBackup:true });

    assert.equal(service.shardStore.manifest.shards.archives.entries[0].path, archiveReference);
    assert.equal(readFileSync(path.join(statePath, archiveReference), "utf8"), archiveContents);
    const reloaded = new YellowDogsLeagueService({ statePath, backupDir:null, now:() => currentTime, rng:() => .37 });
    assert.equal(reloaded.state.season.date, "2026-08-13");
    assert.equal(reloaded.state.archives.length, 1);
    assert.equal(reloaded.state.archives[0].season.status, "completed");
    assert.equal(reloaded.state.archives[0].matchCount, 0);
    assert.equal("matches" in reloaded.state.archives[0], false);
    const fullArchive = reloaded.shardStore.readFullArchive(reloaded.state.archives[0]);
    assert.equal(fullArchive.season.status, "completed");
    assert.ok(fullArchive.playerStats);
    assert.ok(fullArchive.cup);
    assert.ok(Array.isArray(fullArchive.matches));
    assert.equal(reloaded.adminView().archives[0].matchCount, 0);
  } finally {
    rmSync(directory, { recursive:true, force:true });
  }
});

test("杯赛联赛阶段直播轮次在分片存储重载后继续保留", () => {
  const directory = mkdtempSync(path.join(process.cwd(), ".tmp-ydl-sharded-cup-live-"));
  const statePath = path.join(directory, "league-state");
  try {
    const service = new YellowDogsLeagueService({ statePath, backupDir:null, now:() => NOW, rng:() => .37 });
    service.state.teams.forEach((team, index) => {
      team.ownerId = `cup-owner-${index + 1}`;
      team.ownerName = `Cup Player ${index + 1}`;
    });
    service.startCup();
    service.startScheduledCupEvent();

    const eventId = service.state.liveCupRound.eventId;
    const liveMatchCount = service.state.liveCupRound.matches.length;
    const reloaded = new YellowDogsLeagueService({ statePath, backupDir:null, now:() => NOW, rng:() => .37 });

    assert.equal(reloaded.state.liveCupRound.eventId, eventId);
    assert.equal(reloaded.state.liveCupRound.matches.length, liveMatchCount);
    assert.equal(reloaded.state.cup.format, "round-robin-v1");
  } finally {
    rmSync(directory, { recursive:true, force:true });
  }
});
