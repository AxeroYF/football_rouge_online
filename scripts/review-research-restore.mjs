...
const root=process.cwd(),out=path.join(root,'outputs/research-restore-20260908'),data=fs.mkdtempSync(path.join(os.tmpdir(),'ydl-wonder-live-'));
...
try{
 ...
 const originalGeometry=await page.locator('[data-league-magnet]').evaluateAll(nodes=>Object.fromEntries(nodes.map(n=>[n.dataset.leagueMagnet,{x:n.style.left,y:n.style.top}])));
 ...
 const savedBackup=state.tactics.squads.expedition.planSnapshots.__s4V2.researchFormationBackups.position2;
 check('pre-import backup is saved with tactics',Boolean(savedBackup));
 await open();await page.locator('[data-use-research="custom-3"]').click();await page.waitForTimeout(1400);
 ...
 check('research binding survives reload',await page.locator('[data-release-research]').isVisible());
 for(const width of [2514,1600,1280,768,390]){
  await page.setViewportSize({width,height:1050});await page.waitForTimeout(100);
  const boxes=await page.locator('.league-position-tabs button').evaluateAll(nodes=>nodes.map(n=>{const r=n.getBoundingClientRect();return{x:r.x,y:r.y,width:r.width,height:r.height}}));
  check('all five toolbar buttons share a row '+width,boxes.length===5&&boxes.every(b=>Math.abs(b.y-boxes[0].y)<2));
  check('import and release buttons have compact widths '+width,boxes[3].width<150&&boxes[4].width<110);
  await page.screenshot({path:path.join(out,'toolbar-'+width+'.png')});
 }
 await page.setViewportSize({width:1600,height:1050});await page.locator('[data-release-research]').click();await page.waitForTimeout(1400);
 const restoredGeometry=await page.locator('[data-league-magnet]').evaluateAll(nodes=>Object.fromEntries(nodes.map(n=>[n.dataset.leagueMagnet,{x:n.style.left,y:n.style.top}])));
 check('release restores every original player coordinate after switching and reload',JSON.stringify(restoredGeometry)===JSON.stringify(originalGeometry));
 const restoredState=(await(await page.request.get(url+'/api/campaign/state',{headers:{Authorization:'Bearer isolated-wonder-token'}})).json()).state.tactics.squads.expedition.planSnapshots.__s4V2;
 fs.writeFileSync(path.join(out,'restore-state.json'),JSON.stringify({restored:restoredState.positionPresets.position2,expected:savedBackup.positions},null,2));
 assert.deepEqual(restoredState.positionPresets.position2,savedBackup.positions);check('restored coordinates saved to server');
 check('restored reference lines saved to server',JSON.stringify(restoredState.formationLinePresets.position2)===JSON.stringify(savedBackup.lines));
 check('binding and backup removed after restoration',!restoredState.researchFormationIds.position2&&!restoredState.researchFormationBackups.position2);
