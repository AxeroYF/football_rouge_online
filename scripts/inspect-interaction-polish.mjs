...
try {
 ...
 await pageB.locator('#server-players [data-interaction-player="'+actor.id+'"]').click();await pageB.locator('.interaction-action b').first().waitFor();console.log(await pageB.locator('.interaction-action b').first().evaluate(e=>({size:getComputedStyle(e).fontSize,width:innerWidth,styles:[...document.styleSheets].flatMap(s=>{try{return [...s.cssRules]}catch{return []}}).flatMap(r=>r.cssRules?[...r.cssRules]:[r]).filter(r=>{try{return r.selectorText&&e.matches(r.selectorText)&&r.style?.fontSize}catch{return false}}).map(r=>[r.selectorText,r.style.fontSize])})));
