...
const root=process.cwd(),out=path.join(root,'outputs/research-20260908'),data=fs.mkdtempSync(path.join(os.tmpdir(),'ydl-wonder-live-'));
...
try{
 ...
 check('research opens from flat topbar',await trigger.getAttribute('aria-current')==='page');check('all 32 existing subjects',await page.locator('[data-research-topic]').count()===32);check('three branches',await page.locator('.research-branch').count()===3);
 check('actual science capacity visible',Number((await page.locator('[data-research-capacity]').textContent()).replace(/[^0-9.]/g,''))>0);
 check('research explicitly unavailable',await page.locator('.research-start').isDisabled()&&(await window.textContent()).includes('研究预览'));
 check('formation has eleven positions',await page.locator('.research-detail-art .research-player-dot').count()===11);
 await page.screenshot({path:path.join(out,'research-desktop.png')});
 await page.locator('[data-research-branch="tactic"]').click();check('tactics share existing ids',await page.locator('[data-research-topic="style:lowBlock"]').count()===1&&await page.locator('[data-research-topic="tactic:parkBus"]').count()===1);
 await page.locator('[data-research-search]').fill('摆大巴');check('search filters named style',await page.locator('[data-research-topic]').count()===1&&(await page.locator('.research-detail h2').textContent())==='摆大巴');
 await page.locator('[data-research-search]').fill('nothing-matches');check('empty search has no stale details',await page.locator('[data-research-topic]').count()===0&&await page.locator('.research-detail h2').count()===0);
 await page.locator('[data-research-search]').fill('');await page.locator('[data-research-branch="enhancement"]').click();await page.locator('[data-research-topic="enhancement:8"]').click();check('target plus8 is explicit',(await page.locator('.research-detail').textContent()).includes('当前 +7')&&(await page.locator('.research-detail').textContent()).includes('目标 +8'));check('base chance from actual rules',(await page.locator('.research-detail').textContent()).includes('25%'));
 await page.screenshot({path:path.join(out,'research-enhancement.png')});
 await page.locator('[data-research-branch="all"]').click();await page.locator('.research-tree').evaluate(e=>e.scrollTop=210);await page.locator('.research-detail').evaluate(e=>e.scrollTop=130);const scroll=await page.locator('.research-tree').evaluate(e=>e.scrollTop);await page.waitForTimeout(6500);check('polling retains selection',await page.locator('[data-research-topic="enhancement:8"]').getAttribute('aria-pressed')==='true');check('polling retains tree scroll',await page.locator('.research-tree').evaluate(e=>e.scrollTop)===scroll);
 await page.keyboard.press('Escape');check('escape closes and restores focus',await window.isHidden()&&await trigger.evaluate(e=>e===document.activeElement));await trigger.click();await page.locator('#topbar-wonders').click();check('other standard window supersedes research',await window.isHidden());await page.keyboard.press('Escape');
 for(const width of [1280,768,390,320]){await page.setViewportSize({width,height:900});await trigger.click();await page.waitForTimeout(180);check(width+' page fits',await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));check(width+' surface fits',await page.locator('.research-surface').evaluate(e=>{const r=e.getBoundingClientRect();return r.left>=0&&r.right<=innerWidth&&e.scrollWidth<=e.clientWidth+1;}));if(width===390){await page.locator('[data-research-branch="formation"]').click();await page.locator('[data-research-topic="formation:4-4-2"]').click();await page.waitForTimeout(600);check('mobile selection reveals details',await page.locator('.research-detail').evaluate(e=>e.getBoundingClientRect().top<innerHeight));await page.screenshot({path:path.join(out,'research-mobile.png')});await page.locator('[data-research-back]').click();await page.waitForTimeout(600);check('mobile returns to topics',await page.locator('.research-layout').evaluate(e=>e.scrollTop)===0);}await page.keyboard.press('Escape');}
