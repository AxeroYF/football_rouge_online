...
try {
 ...
 const denied=await pageA.request.post(url+'/api/campaign/territory/challenge',{headers:{authorization:'Bearer '+actor.token},data:{territoryId:hill}});check('server rejects attack without war',denied.status()===409&&(await denied.json()).error.includes('宣战'));
 const loc=await click(pageA,'[data-interaction-action="location"]');
 await pageB.locator(`#interaction-notifications [data-interaction-player="${actor.id}"]`).waitFor({timeout:12000});check('recipient sees the request notification');
 await pageB.locator(`#interaction-notifications [data-interaction-player="${actor.id}"]`).click();await respond(pageB,loc.proposalId);
 await rootA.locator('[data-interaction-locate]').waitFor({timeout:12000});check('headquarters visible only after approval',s.diplomacy.details(actor,second.id).location.territoryId===remote.territoryId);
 const friend=await click(pageA,'[data-interaction-action="friendship"]');await respond(pageB,friend.proposalId);await rootA.getByRole('button',{name:'已建立友谊',exact:true}).waitFor({timeout:12000});check('friendship requires and reflects second-player consent');
 await click(pageA,'[data-interaction-action="condemn"]');check('condemnation ends friendship',s.diplomacy.relationship(actor.id,second.id).state==='neutral');await click(pageA,'[data-interaction-action="withdraw-condemnation"]');
 await rootA.locator('[data-interaction-action="trade-form"]').click();
 await rootA.locator('[data-trade-gold="give"]').fill('500');await rootA.locator('[data-trade-gold="take"]').fill('200');
 const give=rootA.locator('[data-trade-card="give"]:not([disabled])').last(),take=rootA.locator('[data-trade-card="take"]:not([disabled])').last();
 const giveId=await give.getAttribute('value'),takeId=await take.getAttribute('value');await give.check();await take.check();
 await pageA.waitForTimeout(3500);check('trade input survives live refresh',await rootA.locator('[data-trade-gold="give"]').inputValue()==='500'&&await rootA.locator(`[data-trade-card="give"][value="${giveId}"]`).isChecked());
 await pageA.screenshot({path:path.join(out,'interaction-trade-desktop.png')});
 const receiverIds=new Set(second.draft.roster.map(p=>p.id));
 const offered=await click(pageA,'[data-interaction-trade] button[type="submit"]');await respond(pageB,offered.proposalId);
 check('trade transfers both cards and nets both payments',actor.gold===99700&&second.gold===100300&&actor.draft.roster.some(p=>p.id===takeId)&&!actor.draft.roster.some(p=>p.id===giveId)&&second.draft.roster.filter(p=>!receiverIds.has(p.id)).length===1);
 await rootA.locator('[data-interaction-action="war-confirm"]').click();await click(pageA,'[data-interaction-action="war"]');
 check('war unlocks the adjacent enemy territory',s.state(actor).attackableTerritoryIds.includes(hill));check('declaration revokes location sharing',s.diplomacy.details(actor,second.id).location===null);
 await pageA.screenshot({path:path.join(out,'interaction-war-desktop.png')});
 const peace=await click(pageA,'[data-interaction-action="peace"]');await respond(pageB,peace.proposalId);check('accepted peace blocks attacks again',!s.state(actor).attackableTerritoryIds.includes(hill));
 await waitEnabled(pageA,'[data-interaction-action="friendly"]');const friendly=await click(pageA,'[data-interaction-action="friendly"]');
 const accepted=await respond(pageB,friendly.proposalId);await pageB.locator('#campaign-broadcast:not([hidden])').waitFor();check('friendly match opens the existing live broadcast');await pageB.screenshot({path:path.join(out,'friendly-live.png')});
 const rosterBefore=JSON.stringify([actor.draft.roster,second.draft.roster]);
 clock+=130000;s.advanceActiveChallenges(clock,{maximumMatches:10,maximumChainsPerMatch:1000});
 check('friendly finishes through V2.1',s.diplomacy.snapshot(actor,accepted.matchId).completed);check('friendly writes no fatigue or injuries to either roster',JSON.stringify([actor.draft.roster,second.draft.roster])===rosterBefore);
 await pageB.keyboard.press('Escape');await rootB.waitFor({state:'visible'});check('leaving broadcast returns to the same interaction');await rootB.locator(`[data-friendly-watch="${accepted.matchId}"]`).click();await pageB.locator('#campaign-broadcast:not([hidden])').waitFor();await pageB.screenshot({path:path.join(out,'friendly-report.png')});check('completed friendly report remains accessible');
 await pageA.setViewportSize({width:390,height:844});await rootA.locator('.interaction-scroll').evaluate(el=>el.scrollTop=0);await pageA.screenshot({path:path.join(out,'interaction-mobile.png')});check('mobile actions precede the pitch',await rootA.locator('.interaction-actions').evaluate(el=>el.getBoundingClientRect().top<document.querySelector('.interaction-pitch').getBoundingClientRect().top));
 check('mobile dialog has no horizontal overflow',await pageA.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
 await pageA.keyboard.press('Escape');await pageA.reload();await pageA.locator('#server-players:not([hidden])').waitFor({timeout:60000});await pageA.waitForFunction(()=>document.querySelector('#map-loader')?.classList.contains('is-ready'),null,{timeout:60000});await pageA.locator('[data-players-toggle]').click();await pageA.screenshot({path:path.join(out,'server-players-mobile.png')});check('mobile server list expands from compact header');
 check('no JavaScript exceptions',errors.length===0);
 if(fs.existsSync(path.join(out,'browser-failure.json')))fs.renameSync(path.join(out,'browser-failure.json'),path.join(out,'initial-fixture-assertion.json'));
 fs.writeFileSync(path.join(out,'browser-results.json'),JSON.stringify({passed:checks.length,checks,errors,isolation:'Independent temporary campaign, two browser sessions, real HTTP handlers/static assets; controlled clock only for friendly finish.'},null,2));
