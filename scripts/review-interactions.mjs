...
const root=process.cwd(),out=path.join(root,'outputs/interactions-20260909'),data=fs.mkdtempSync(path.join(os.tmpdir(),'ydl-interactions-'));
...
try {
 ...
 await pageA.setViewportSize({width:390,height:844});await pageA.screenshot({path:path.join(out,'interaction-mobile.png')});check('mobile dialog has no horizontal overflow',await pageA.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
