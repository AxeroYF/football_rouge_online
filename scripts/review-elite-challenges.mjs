...
const root=process.cwd(),out=path.join(root,'outputs/elite-two-level-20260909'),data=fs.mkdtempSync(path.join(os.tmpdir(),'ydl-v21-review-'));
...
try{
 ...
 const response=page.waitForResponse(r=>r.url().endsWith('/api/campaign/elite/begin')&&r.request().method()==='POST');await page.locator('[data-elite-begin]').click();const started=await (await response).json();assert.ok(started.challengeId,JSON.stringify(started));assert.equal(started.state.wallet.gold,gold-5000);
 ...
 await context.addInitScript(()=>localStorage.setItem('yellowdogs-chronicles-token','elite-winner-token'));await page.reload();await page.waitForFunction(()=>window.__eliteReview?.state()?.playerId==='elite-winner',null,{timeout:60000});await page.locator('#topbar-elite').click();await page.waitForSelector('[data-elite-claim]');assert.equal(await page.locator('[data-elite-claim]').count(),3);
 await page.screenshot({path:path.join(out,'elite-reward.png')});await page.locator('[data-elite-watch]').click();await page.locator('#campaign-broadcast:not([hidden])').waitFor();await page.screenshot({path:path.join(out,'elite-report.png')});await page.keyboard.press('Escape');await page.waitForSelector('[data-elite-claim]');const choice=page.waitForResponse(r=>r.url().endsWith('/api/campaign/elite/claim')&&r.request().method()==='POST');await page.locator('[data-elite-claim]').first().click();const claimed=await (await choice).json();assert.ok(claimed.playerId,JSON.stringify(claimed));await page.waitForFunction(()=>document.querySelectorAll('[data-elite-claim]').length===0);
 ...
 assert.deepEqual(errors,[]);fs.writeFileSync(path.join(out,'report.json'),JSON.stringify({clubs:info.elite.clubs.map(c=>({name:c.name,formation:c.formation,average:c.average,fans:c.fans})),challengeId:id,rewardPlayer:claimed.playerId,errors,checks:['18 badges in five tiers [1,6,5,4,2]','catalog and detail navigation','back restores list scroll','all badge images load','desktop and mobile','5000 debit','live V2.1 broadcast','full fitness match copy','regular capture blocked','restart retains reward','one base card delivered']},null,2));console.log('Elite browser review passed: '+out);
