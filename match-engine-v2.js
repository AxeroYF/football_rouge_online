...
function resolveShot(match, teamIndex, chain, options = {}) {
  ...
  if (!options.taker && !longShot && type === "cross") {
    shooter = pick(match, candidates, (player) => effectiveMetric(match, teamIndex, player, { heading:0.42, jumping:0.22, strength:0.18, offBall:0.18 }) + (Number(snapshotPlayer(match, teamIndex, player.id)?.heightCm ?? player.heightCm ?? 180) - 180) * 0.4);
  }
...
export function v2SelectSetPieceTaker(match, teamIndex, kind = "freeKick") {
  const candidates = activePlayers(match.teams[teamIndex]).filter((player) => roleGroup(player.assignedRole ?? player.role) !== "GK");
