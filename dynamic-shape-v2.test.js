...
function buildShape(team, options = {}) {
  return buildV21DynamicTeamShape({
    ...
    dimensions:options.dimensions ?? resolveV2TacticalDimensions(team.tactic, team.style),
...
test("mentality changes attacking and defensive depth by tactical identity", () => {
  const baseTeam = makeTeam("mentality");
  const profiles = {
    allOutAttack:{ mentality:92, counterAttack:36, pressing:72, compactness:42, defensiveLine:50 },
    counterAttack:{ mentality:34, counterAttack:62, pressing:42, compactness:64, defensiveLine:50 },
    parkBus:{ mentality:20, counterAttack:64, pressing:30, compactness:72, defensiveLine:50 },
  };
  const shapes = Object.fromEntries(Object.entries(profiles).map(([name, dimensions]) => [name, {
    attack:buildShape(structuredClone(baseTeam), { dimensions, stage:"chance" }),
    defense:buildShape(structuredClone(baseTeam), { dimensions, teamIndex:1, attackingTeamIndex:0, stage:"chance" }),
  }]));
  const role = (shape, assignedRole) => playerByRole(shape, assignedRole).targetPosition.y;
  assert.ok(role(shapes.allOutAttack.attack, "CM") < role(shapes.counterAttack.attack, "CM"));
  assert.ok(role(shapes.counterAttack.attack, "ST") < role(shapes.counterAttack.attack, "CM"));
  assert.ok(role(shapes.parkBus.attack, "ST") > role(shapes.counterAttack.attack, "ST"));
});

test("defensive line height affects backline in both phases but not cover duty", () => {
  const high = { mentality:50, counterAttack:50, pressing:50, compactness:50, defensiveLine:85 };
  const low = { ...high, defensiveLine:15 };
  const highAttack = buildShape(makeTeam("high-line"), { dimensions:high, stage:"chance" });
  const lowAttack = buildShape(makeTeam("low-line"), { dimensions:low, stage:"chance" });
  const highDefense = buildShape(makeTeam("high-line-defense"), { dimensions:high, teamIndex:1, attackingTeamIndex:0, stage:"chance" });
  const lowDefense = buildShape(makeTeam("low-line-defense"), { dimensions:low, teamIndex:1, attackingTeamIndex:0, stage:"chance" });
  assert.ok(playerByRole(highAttack, "CB").targetPosition.y < playerByRole(lowAttack, "CB").targetPosition.y);
  assert.ok(playerByRole(highDefense, "CB").targetPosition.y < playerByRole(lowDefense, "CB").targetPosition.y);
  assert.ok(playerByRole(highAttack, "CM").targetPosition.y < playerByRole(lowAttack, "CM").targetPosition.y);
  assert.ok(playerByRole(highDefense, "CM").targetPosition.y < playerByRole(lowDefense, "CM").targetPosition.y);
  assert.ok(playerByRole(highAttack, "ST").targetPosition.y < playerByRole(lowAttack, "ST").targetPosition.y);
  assert.ok(playerByRole(highDefense, "ST").targetPosition.y < playerByRole(lowDefense, "ST").targetPosition.y);

  const coverTeam = makeTeam("cover-line");
  coverTeam.players.find((player) => player.role === "CB").tacticalDuty = "coverDefender";
  const coverHigh = buildShape(coverTeam, { dimensions:high, stage:"chance" });
  const coverLow = buildShape(coverTeam, { dimensions:low, stage:"chance" });
  assert.ok(Math.abs(playerByRole(coverHigh, "CB").targetPosition.y - playerByRole(coverLow, "CB").targetPosition.y) < 1);
});

test("forward duties control attacking movement direction and depth", () => {
  const makeDutyTeam = (duty) => {
    const team = makeTeam(`forward-duty-${duty}`);
    const striker = team.players.find((player) => player.role === "ST");
    striker.tacticalDuty = duty;
    return team;
  };
  const dimensions = { mentality:50, counterAttack:50, pressing:50, compactness:50, defensiveLine:50 };
  const targetAttack = buildShape(makeDutyTeam("targetForward"), { dimensions, stage:"chance" });
  const targetDefense = buildShape(makeDutyTeam("targetForward"), { dimensions, teamIndex:1, attackingTeamIndex:0, stage:"chance" });
  const deepAttack = buildShape(makeDutyTeam("deepLyingForward"), { dimensions, stage:"chance" });
  const defaultAttack = buildShape(makeDutyTeam(null), { dimensions, stage:"chance" });
  const striker = (shape) => playerByRole(shape, "ST");
  assert.ok(Math.abs(striker(targetAttack).targetPosition.y - striker(targetAttack).basePosition.y) < 4);
  assert.ok(striker(targetDefense).targetPosition.y > striker(targetDefense).basePosition.y + 5);
  assert.ok(striker(deepAttack).targetPosition.y > striker(deepAttack).basePosition.y);
  assert.ok(striker(deepAttack).targetPosition.y > striker(defaultAttack).targetPosition.y);
});

test("byline wingers keep advancing instead of being pulled behind the striker", () => {
  const defaultTeam = makeTeam("default-winger");
  const bylineTeam = makeTeam("byline-winger");
  bylineTeam.players.filter((player) => ["LW", "RW"].includes(player.role)).forEach((player) => { player.tacticalDuty = "bylineWinger"; });
  const dimensions = { mentality:50, counterAttack:50, pressing:50, compactness:50, defensiveLine:50 };
  const defaultShape = buildShape(defaultTeam, { dimensions, stage:"chance", ballLane:"leftHalfSpace" });
  const bylineShape = buildShape(bylineTeam, { dimensions, stage:"chance", ballLane:"leftHalfSpace" });
  const defaultLeft = playerByRole(defaultShape, "LW");
  const bylineLeft = playerByRole(bylineShape, "LW");
  assert.ok(bylineLeft.targetPosition.y < defaultLeft.targetPosition.y - 2);
  assert.ok(bylineLeft.targetPosition.y < bylineLeft.basePosition.y);
});
