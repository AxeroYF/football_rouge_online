...
test("card recovery returns deletion deltas without rebuilding the full team view", () => {
  const service = new YellowDogsLeagueService({ statePath:null, now:() => NOW, rng:() => .37 });
  const user = account("compact-card-recovery");
  const team = join(service, user);
  const playerId = team.rosterIds.find((id) => !isXPlayer(REAL_PLAYER_BY_ID[id]) && REAL_PLAYER_BY_ID[id].grade !== "S");
  const material = service.grantS4Card(team, playerId, { grantOwnership:false, upgradeLevel:1, acquisitionSource:"performance-test" });
  service.ownTeamView = () => { throw new Error("compact card recovery must not rebuild ownTeam"); };

  const result = service.releaseCards(user, [material.id], { compact:true });

  assert.equal(result.compact, true);
  assert.equal("ownTeam" in result, false);
  assert.deepEqual(result.removedS4CardIds, [material.id]);
  assert.deepEqual(result.removedS4PlayerIds, []);
  assert.deepEqual(result.returnedS4OwnershipPlayerIds, []);
  assert.equal(result.s4PlayerDeltas[0].playerId, playerId);
  assert.equal(result.cardRecoveryResult.cardCount, 1);
  assert.ok(Buffer.byteLength(JSON.stringify(result)) < 25_000);
});

test("ownership recovery returns card, ownership, roster, and player deletion deltas", () => {
  const service = new YellowDogsLeagueService({ statePath:null, now:() => NOW, rng:() => .37 });
  const user = account("compact-ownership-recovery");
  const team = join(service, user);
  const playerIds = team.rosterIds
    .filter((id) => !isXPlayer(REAL_PLAYER_BY_ID[id]) && REAL_PLAYER_BY_ID[id].grade !== "S")
    .slice(-2);
  const [retainedPlayerId, removedPlayerId] = playerIds;
  const retainedBaseCard = service.representativeCard(user.id, retainedPlayerId);
  const removedBaseCard = service.representativeCard(user.id, removedPlayerId);
  const retainedCard = service.grantS4Card(team, retainedPlayerId, { grantOwnership:false, upgradeLevel:5, acquisitionSource:"performance-test" });
  service.ownTeamView = () => { throw new Error("compact ownership recovery must not rebuild ownTeam"); };

  const result = service.returnOwnerships(user, playerIds, { compact:true });

  assert.equal(result.compact, true);
  assert.equal("ownTeam" in result, false);
  assert.deepEqual(new Set(result.removedS4CardIds), new Set([retainedBaseCard.id, removedBaseCard.id]));
  assert.deepEqual(result.removedS4PlayerIds, [removedPlayerId]);
  assert.deepEqual(new Set(result.returnedS4OwnershipPlayerIds), new Set(playerIds));
  assert.ok(result.s4RosterDelta);
  assert.ok(!result.s4RosterDelta.rosterPlayerIds.includes(removedPlayerId));
  assert.ok(result.s4RosterDelta.rosterPlayerIds.includes(retainedPlayerId));
  assert.equal(service.playerCards(user.id, retainedPlayerId)[0].id, retainedCard.id);
  assert.equal(result.ownershipRecoveryResult.playerCount, 2);
  assert.ok(Buffer.byteLength(JSON.stringify(result)) < 100_000);
});
...
test("large pack and market interfaces mount bounded collections", () => {
  ...
  assert.match(appSource, /removedS4CardIds/);
  assert.match(appSource, /returnedS4OwnershipPlayerIds/);
  assert.match(appSource, /removedS4PlayerIds/);
  assert.match(appSource, /s4RosterDelta/);
  assert.match(appSource, /leagueRequest\("\/cards\/release",[\s\S]*?\{ render:false \}\)[\s\S]*?renderLeague\(\)/);
  assert.match(appSource, /leagueRequest\("\/ownership\/return-batch",[\s\S]*?\{ render:false \}\)[\s\S]*?renderLeague\(\)/);
});
