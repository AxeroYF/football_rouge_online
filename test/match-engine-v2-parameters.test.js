...
test("V2所有单项能力重新硬限制为99", () => {
  assert.equal(V2_MATCH_PARAMETERS.ability.maximum, 99);
  assert.equal(Object.hasOwn(V2_MATCH_PARAMETERS.ability, "overflowThreshold"), false);
  assert.equal(Object.hasOwn(V2_MATCH_PARAMETERS.ability, "overflowRetention"), false);
  assert.equal(v2EngineAttributeValue(99), 99);
  assert.equal(v2EngineAttributeValue(109), 99);
  assert.equal(v2EngineAttributeValue(119), 99);
});
...
test("V2战术预设完整覆盖连续战术维度", () => {
  ...
  assert.equal(V2_MATCH_PARAMETERS.dynamicShape.modelVersion, "dynamic-shape-v2.1-stable.9");
...
test("V2参数覆盖兼容分片状态中的代理数组", () => {
  const stages = new Proxy([...V2_MATCH_PARAMETERS.chain.stages], {});
  const resolved = resolveV2MatchParameters({ chain:{ stages } });
  assert.deepEqual(resolved.chain.stages, V2_MATCH_PARAMETERS.chain.stages);
  assert.notEqual(resolved.chain.stages, stages);
});
...
test("V2参数清单公开版本、20个空间区域和唯一结果负责人", () => {
  ...
  assert.equal(V2_MATCH_PARAMETERS.events.roughPlay.foulInjuryMultiplier, 2.2);
  assert.ok(V2_MATCH_PARAMETERS.events.highPressInjury.minimumMultiplier > 1);
  assert.ok(V2_MATCH_PARAMETERS.events.highPressInjury.maximumMultiplier > V2_MATCH_PARAMETERS.events.highPressInjury.minimumMultiplier);
  ...
  assert.equal(V2_MATCH_PARAMETERS.environment.weatherWeights.superStorm, 1);
  assert.deepEqual(V2_MATCH_PARAMETERS.environment.superStormStopMinuteRange, { minimum:61, maximum:89 });
