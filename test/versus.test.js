...
test("旧版比赛详情会补回球员能力并生成分散的阵型坐标", () => {
  ...
  const detail = hydrateHistoricalMatchDetail({
    id:"legacy-detail",
    teams:[{ name:"旧版球队", formation:"4-3-3", players:players.map((player) => ({ id:player.id, name:player.name, role:player.role, rating:6.5 })) }],
  });
  const team = detail.teams[0];
  assert.equal(team.players.length, 11);
  assert.ok(team.players.every((player) => player.overall === REAL_PLAYER_BY_ID[player.id].overall));
  assert.ok(team.players.every((player) => Number.isFinite(player.position.x) && Number.isFinite(player.position.y)));
...
test("玩家ID绑定、自定义分享码与历史战绩会持久累计", () => {
  ...
  assert.ok(detail.teams.every((team) => team.players.length === 11 && team.positions));
  assert.ok(detail.teams.every((team) => team.players.every((player) =>
...
test("玩家可以使用任意非空昵称和密码注册并登录", () => {
  const service = new VersusRoomService({ accountsPath: null });
  const nickname = "球迷 甲@主场";
  const password = "空 格/符号/中文!";
  const registered = service.register(nickname, password);
  assert.equal(registered.profile.nickname, nickname);
  assert.match(registered.profile.id, /^P-[A-F0-9]{10}$/);
  assert.equal(service.accounts.get(registered.profile.id.toLowerCase()).passwordHash.includes(password), false);
  assert.throws(() => service.register(nickname, "另一个密码"), /已经注册/);
  assert.throws(() => service.login(nickname, "错误密码"), /昵称或密码错误/);
  const loggedIn = service.login(nickname, password);
  assert.equal(loggedIn.profile.id, registered.profile.id);
  assert.equal(loggedIn.accountToken, registered.accountToken);
  assert.throws(() => service.register("", password), /昵称不能为空/);
  assert.throws(() => service.register("新玩家", ""), /密码不能为空/);
});

test("旧版本地账号可在原设备设置密码并保留战绩", () => {
  const service = new VersusRoomService({ accountsPath: null });
  const legacy = service.bindAccount(null, null, "旧版玩家");
  assert.equal(legacy.profile.passwordSet, false);
  service.accounts.get(legacy.profile.id.toLowerCase()).summary.wins = 7;
  assert.throws(() => service.register("旧版玩家", "新密码"), /原设备/);
  const upgraded = service.register("旧版玩家", "新密码", legacy.accountToken);
  assert.equal(upgraded.profile.passwordSet, true);
  assert.equal(upgraded.profile.summary.wins, 7);
  assert.equal(service.login("旧版玩家", "新密码").profile.id, legacy.profile.id);
});

test("重复昵称的旧账号可以改用唯一昵称升级", () => {
  const service = new VersusRoomService({ accountsPath: null });
  const first = service.bindAccount(null, null, "同名玩家");
  const second = service.bindAccount(null, null, "同名玩家");
  assert.throws(() => service.register("同名玩家", "密码", second.accountToken), /已经被占用/);
  const upgraded = service.register("同名玩家-2", "密码", second.accountToken);
  assert.equal(upgraded.profile.id, second.profile.id);
  assert.equal(upgraded.profile.nickname, "同名玩家-2");
  assert.equal(service.login("同名玩家-2", "密码").profile.id, second.profile.id);
...
test("S4十一人对战拥有含DLC的分位置真实球员池和稳定评级梯次", () => {
  ...
  assert.deepEqual(
    ...
    [
      ["s4-dlc2-20260803-001", "阿尔弗雷多·迪斯蒂法诺", 93, "S", true],
      ["s4-dlc2-20260803-074", "古蒂", 88, "A", true],
    ],
  );
  assert.deepEqual(
...
test("同侧边后卫客串边翼卫时减益很小并提供边路支持", () => {
  const leftBack = REAL_PLAYER_POOLS.DEF.find((player) => player.role === "LB");
  const rightBack = REAL_PLAYER_POOLS.DEF.find((player) => player.role === "RB");
  assert.ok(versusPositionFit(leftBack, "LWB") >= 0.97);
  assert.ok(versusPositionFit(rightBack, "RWB") >= 0.97);
  assert.ok(versusWingbackSupport(leftBack, "LWB") > 0.5);
  assert.ok(versusWingbackSupport(rightBack, "RWB") > 0.5);
  assert.equal(versusWingbackSupport(rightBack, "LWB"), 0);
});
...
test("选秀完成门将选择后会锁定门将池", () => {
  let now = 900;
  const service = new VersusRoomService({ rng: () => 0.19, now: () => now++, accountsPath: null });
...
test("房间选择球员后自动绑定一张特性并进入双方战术阶段", () => {
  let now = 1_000;
  const service = new VersusRoomService({ rng: () => 0.17, now: () => now++, accountsPath: null });
  const host = service.create("甲");
...
test("双方选秀都获得独立且必定出现的S级球员保底", () => {
  let now = 1_250;
  const service = new VersusRoomService({ rng: () => 0.41, now: () => now++, accountsPath: null });
  const host = service.create("甲");
  const guest = service.join(host.room.code, "乙");
...
test("开发者入口支持单人完整流程和快速开赛", () => {
  let now = 1_500;
  const service = new VersusRoomService({ rng: () => 0.27, now: () => now++, accountsPath: null });
...
test("阵容种子完整保存球员、特性、站位和战术并拒绝篡改", () => {
  let now = 1_700;
  const service = new VersusRoomService({ rng: () => 0.29, now: () => now++, accountsPath: null });
...
test("赛后阵容可以导入新房间跳过选秀并在比赛中公开来源标记", () => {
  let sourceNow = 2_000;
  const sourceService = new VersusRoomService({ rng: () => 0.33, now: () => sourceNow, accountsPath: null });
  ...
  let targetNow = 5_000;
  const targetService = new VersusRoomService({ rng: () => 0.38, now: () => targetNow++, accountsPath: null });
...
test("双方只有提交有效十一人阵型后才能同时准备", () => {
  let now = 2_000;
  const service = new VersusRoomService({ rng: () => 0.31, now: () => now++, accountsPath: null });
  const host = service.create("甲");
  const guest = service.join(host.room.code, "乙");
  for (let round = 0; round < VERSUS_TEAM_SIZE; round += 1) {
...
test("战术暂停冻结比赛时钟且每方只能使用一次", () => {
  ...
  assert.deepEqual(match.teams[0].positions, attackingPositions);
  assert.ok(match.teams[0].players.filter((player) => player.active).every((player) => player.boardPosition.y === 18 && player.assignedRole === (player.boardPosition.x < 38 ? "LW" : player.boardPosition.x > 62 ? "RW" : "ST")));
