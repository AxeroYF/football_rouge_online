...
test("persisted bootstrap admin is synchronized to the configured default password", () => {
  ...
  try {
    ...
    assert.equal(restarted.login("admin", DEFAULT_ADMIN_PASSWORD).profile.username, "admin");
    assert.throws(() => restarted.login("admin", "old-password"), /账号或密码错误/);
  } finally { fs.rmSync(root, { recursive:true, force:true }); }
});

test("admin login page is a single card with the YellowDogs logo and configuration hint", () => {
