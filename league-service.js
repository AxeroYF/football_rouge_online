...
const MANAGER_ONLINE_WINDOW_MS = 60 * 60 * 1000;
const MANAGER_INACTIVE_MATCH_MULTIPLIER = 0.97;
...
const BALLON_DOR_SCORING = Object.freeze({ goal:4, assist:3, rating:20, appearance:0.25, leagueChampion:35, leagueRunnerUp:15, cupChampion:30, cupRunnerUp:10 });
...
const MATCH_PREDICTION_MAX_STAKE_BY_CATEGORY = Object.freeze({ halfFull:5000 });
...
function publicTeam(team, includeRoster = false) {
  return {
    ...
    roster:includeRoster ? team.rosterIds.map((id) => ({ ...playerSummary(REAL_PLAYER_BY_ID[id]), state:{ fitness:100, suspension:0, cupSuspension:0, injuryRounds:0, ...(team.playerState[id] ?? {}) }, starter:team.preferredStarterIds.includes(id), rosterOverlimitLocked:Boolean(team.playerState?.[id]?.rosterOverlimitLocked), listed:false })) : undefined,
...
export class YellowDogsLeagueService {
  constructor(options = {}) {
    ...
    this.accountLastSeenAt = typeof options.accountLastSeenAt === "function" ? options.accountLastSeenAt : null;
  ...
  setAccountActivityProvider(provider) {
    this.accountLastSeenAt = typeof provider === "function" ? provider : null;
    return this;
  }

  managerActivityMatchMultiplier(team, startedAt = this.now(), competition = "league") {
    if (!team?.ownerId || !["league", "cup"].includes(competition) || !this.accountLastSeenAt) return 1;
    const lastSeenAt = Number(this.accountLastSeenAt(team.ownerId));
    if (!Number.isFinite(lastSeenAt) || lastSeenAt <= 0) return MANAGER_INACTIVE_MATCH_MULTIPLIER;
    const inactiveFor = Math.max(0, Number(startedAt) - lastSeenAt);
    return inactiveFor <= MANAGER_ONLINE_WINDOW_MS
      ? 1
      : MANAGER_INACTIVE_MATCH_MULTIPLIER;
  }
  ...
  rosterEnforcementPreview() {
    ...
    const players = this.state.teams.filter((team) => team.ownerId).map((team) => {
      ...
      const lockedPlayerIds = [...(team.rosterEnforcement?.lockedPlayerIds ?? [])];
      return {
        accountId:team.ownerId, ownerName:team.ownerName, teamId:team.id, teamName:team.name,
        rosterSlotsUsed, rosterLimit, excessCount,
        alreadyApplied:Boolean(team.rosterEnforcement?.overLimit), lockedPlayerIds,
  ...
  applyRosterEnforcement(options = {}) {
    ...
    for (const team of this.state.teams.filter((entry) => entry.ownerId)) {
      ...
      for (const playerId of families) {
        const state = team.playerState[playerId] ?? (team.playerState[playerId] = { fitness:100, suspension:0, cupSuspension:0, injuryRounds:0 });
  ...
  createFixtureMatch(fixture, roundNumber, startedAt = this.now(), options = {}) {
    ...
    const managerActivityMultipliers = teams.map((team) => this.managerActivityMatchMultiplier(team, startedAt, competitionMode));
    const seats = teams.map((team, index) => {
      ...
      return { name:team.name, players:[...starters, ...substitutes], positions:positions[index], positionPresets:positionPresets[index], formationLinePresets:formationLinePresets[index], tactic:tacticalPlans.opening?.tactic ?? team.tactic, style:tacticalPlans.opening?.style ?? team.style, tacticalPlans, bondCatalog:S4_BOND_CATALOG, attackFocus:team.attackFocus, defenseFocus:team.defenseFocus, preserveFitness:true, managerActivityMultiplier:managerActivityMultipliers[index] };
    });
  ...
  ballonDorCandidates() {
    ...
    return [...candidates.values()].filter((candidate) => candidate.appearances >= BALLON_DOR_MINIMUM_APPEARANCES && this.representativeCard(candidate.ownerId, candidate.playerId)).map((candidate) => {
      ...
      const placements = { leagueChampion, leagueRunnerUp, cupChampion, cupRunnerUp };
      ...
      return {
        ...
        champions:{ league:placements.leagueChampion, cup:placements.cupChampion },
        runnerUps:{ league:placements.leagueRunnerUp, cup:placements.cupRunnerUp },
        breakdown,
  ...
  settleBallonDor() {
    ...
    const championText = [winner.champions.league ? "联赛冠军" : null, winner.runnerUps.league ? "联赛亚军" : null, winner.champions.cup ? "杯赛冠军" : null, winner.runnerUps.cup ? "杯赛亚军" : null].filter(Boolean).join("、") || "无冠亚军加成";
    ...
    this.state.teams.filter((team) => team.ownerId).forEach((team) => {
      ...
      this.pushInbox(team, {
        ...
        body:`本次评选合并联赛与杯赛正式数据，参评门槛为至少${BALLON_DOR_MINIMUM_APPEARANCES}次出场，杯赛表现按${BALLON_DOR_CUP_WEIGHT}倍计入。${winner.playerName}的冠亚军情况：${championText}。得分构成为：进球${winner.breakdown.goals}分、助攻${winner.breakdown.assists}分、评分${winner.breakdown.rating}分、出场稳定性${winner.breakdown.appearances}分、联赛冠军${winner.breakdown.leagueChampion}分、联赛亚军${winner.breakdown.leagueRunnerUp}分、杯赛冠军${winner.breakdown.cupChampion}分、杯赛亚军${winner.breakdown.cupRunnerUp}分。金球积分前三名：${podiumText}。${rewarded ? `你持有该球员卡，${BALLON_DOR_OWNER_REWARD.toLocaleString("zh-CN")}金币奖励已经到账。` : "获奖球员卡将永久累计一枚金色地球标识。"}`,
