...
test("season Ballon d'Or selects one player's specific card without merging the same player across managers", () => {
  ...
  assert.deepEqual(result.winner.runnerUps, { league:false, cup:false });
  ...
  assert.deepEqual(runnerUp.runnerUps, { league:true, cup:true });
...
test("official matches keep recent activity neutral and apply hidden multi-round inactivity penalty", () => {
  let lastSeenAt = NOW;
  const service = new YellowDogsLeagueService({
    statePath:null,
    now:() => NOW,
    rng:() => .37,
    accountLastSeenAt:() => lastSeenAt,
  });
  const user = account("activity-modifier-owner", "Activity Modifier Owner");
  join(service, user, "Activity Modifier FC");
  const team = service.accountTeam(user.id);
  const fixture = service.state.rounds[0].fixtures.find((entry) => entry.homeId === team.id || entry.awayId === team.id);
  const ownIndex = fixture.homeId === team.id ? 0 : 1;
  const opponentIndex = 1 - ownIndex;
  ...
  const online = service.createFixtureMatch(fixture, 1, NOW, { competitionMode:"league" }).match;
  assert.equal(online.teams[ownIndex].managerActivityMultiplier, 1);
  assert.equal(online.teams[opponentIndex].managerActivityMultiplier, 1);

  lastSeenAt = NOW - 60 * 60 * 1000 - 1;
  const inactive = service.createFixtureMatch(fixture, 1, NOW, { competitionMode:"cup" }).match;
  assert.equal(inactive.teams[ownIndex].managerActivityMultiplier, .97);

  const friendly = service.createFixtureMatch(fixture, 1, NOW, { competitionMode:"friendly" }).match;
  assert.equal(friendly.teams[ownIndex].managerActivityMultiplier, 1);

  const neutralPlayer = snapshots(friendly)[ownIndex].players[0];
  const onlinePlayer = snapshots(online)[ownIndex].players.find((player) => player.id === neutralPlayer.id);
  const inactivePlayer = snapshots(inactive)[ownIndex].players.find((player) => player.id === neutralPlayer.id);
  const [attribute, neutralValue] = Object.entries(neutralPlayer.attributes).find(([, value]) => Number.isFinite(Number(value)) && Number(value) <= 90);
  assert.equal(onlinePlayer.attributes[attribute], Number(neutralValue));
  assert.equal(inactivePlayer.attributes[attribute], Number(Math.max(1, Number(neutralValue) * .97).toFixed(2)));

  const publicMatch = publicYdlLeagueV2Match(online, NOW, ownIndex, true);
  assert.equal(JSON.stringify(publicMatch).includes("managerActivity"), false);
  assert.equal(Object.hasOwn(publicMatch.teams[ownIndex], "managerActivityMultiplier"), false);
});
