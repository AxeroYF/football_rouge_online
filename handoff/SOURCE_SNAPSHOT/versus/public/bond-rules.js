...
function structureBond(id, name, members, bonus, count = members.length) {
  return {
    id:`structure:${id}`,
    type:"structure",
    label:"阵容结构",
...
function evaluateStructureBonds(lineup, roles) {
  ...
  if (defenders.length >= 6) bonds.push(structureBond("steel-defense", "钢铁防线", defenders, .05));
  const attackers = lineup.filter((player) => assignedGroup(player, roles) === "ATT");
  if (attackers.length >= 5) bonds.push(structureBond("blow-them-up", "跟他们爆了", attackers, .04));
  const midfielders = lineup.filter((player) => assignedGroup(player, roles) === "MID");
  if (midfielders.length >= 6) bonds.push(structureBond("mahjong-together", "一起打麻将", midfielders, .05));
  return bonds;
}
...
export function createS4BondCatalog(players, minimum = S4_BOND_POOL_MINIMUM) {
  return bondTypes.flatMap(({ type, field, label }) => {
    ...
    players.filter((player) => !player?.xPlayer).forEach((player) => {
...
export function applyS4BondBonuses(players, bonds) {
  ...
  return (players ?? []).map((player) => {
    ...
    return {
      ...
      ydlBondIds:appliedBonds.filter((bond) => bond.memberIds.includes(player.id)).map((bond) => bond.id),
    };
  });
}
